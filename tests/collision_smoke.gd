...
func _run() -> void:
    ...
    player.global_position = Vector2(1030, 455)
    Input.action_press("move_right")
    for frame_index in 50:
        await physics_frame
    Input.action_release("move_right")
    assert(player.global_position.x < 1095.0, "Village castle did not block movement")
    player.global_position = Vector2(998, 675)
    Input.action_press("move_up")
    for frame_index in 54:
        await physics_frame
    Input.action_release("move_up")
    assert(player.global_position.y < 510.0, "Castle ramp was not traversable")
    print("COLLISION PASS: river, bridge, castle foundation, castle ramp")
    quit(0)
